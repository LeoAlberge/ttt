...
from src.python.training.losses import DiceSegmentationLoss, FocalLoss
...
class LossesTestCase(unittest.TestCase):
    ...
    def test_dice_loss_with_equal_masks(self):
        ...
        loss_fn = DiceSegmentationLoss(reduction="none", apply_softmax=False,
    ...
    def test_dice_loss_with_ignore_background(self):
        ...
        loss_fn_ignore_bg = DiceSegmentationLoss(reduction="mean", apply_softmax=False,
                                                 ignore_background=False)
        l = loss_fn_ignore_bg(pred, gt)
        np.testing.assert_almost_equal(l.numpy(), 0.2)

        loss_fn_ignore_bg_true = DiceSegmentationLoss(reduction="mean", apply_softmax=False,
    ...
    def test_dice_loss_with_one_batch_zero(self):
        ...
        l = DiceSegmentationLoss(reduction="mean", apply_softmax=False, ignore_background=False)(
            pred, gt)
